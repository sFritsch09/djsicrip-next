...
export default function MusicSection() {
  ...
  const getCovers = async () => {
    const covers = await pb
      .collection("djsicrip_cover")
      .getList(1, 50, { $autoCancel: false });
    setCovers(covers);
  };
  useEffect(() => {
    let ignore = false;
  ...
  return (
    <Fragment>
      <LogoWrapper>
        <ToolTip
          onMouseMove={(e) => setXPosition(e.clientX)}
          $position={xPosition}
        >
          <Link href="/booking">
      ...
      <MusicContainer>
        <MusicWrapper>
          <NavContainer>
            <NavItem
              $active={status === "house"}
              onClick={() => handleClick("showA")}
            >
              House
            </NavItem>
            <NavItem
              $active={status === "classic"}
              onClick={() => handleClick("showB")}
            >
              Classics
            </NavItem>
            <NavItem
              $active={status === "party"}
              onClick={() => handleClick("showC")}
            >
              Party
            </NavItem>
            <NavItem
              $active={status === "hip hop"}
              onClick={() => handleClick("showD")}
            >
              Hip Hop
            </NavItem>
          </NavContainer>
          ...
          {covers.items?.map((item) =>
            playing.includes(item.id) ? (
              <Fragment key={item.id}>
                <Image
                  src={
                    `https://pb.techchase.de/api/files/djsicrip_cover/${item.id}/${item.cover}` ??
                    "https://pb.techchase.de/api/files/baa8qw390ee8nzl/szi9lgoxtjvioih/crip_beatz_klml7_yycc_c_IlxhX4srxo.jpg"
                  }
                  alt="Cover"
                  width={80}
                  height={80}
